...
let rec draw_control parent_x parent_y parent_w parent_h = function
  | Empty -> (0, 0)
  | Border (r, c, t, d) ->
      let w, h = draw_control parent_x parent_y parent_w parent_h d in
      let rect =
  ...
  | Rect (w, h, r, c, d) ->
      ...
      let _ = draw_control parent_x parent_y w h d in
      (w, h)
  | Column lst ->
      let _, w, h =
        List.fold_left
          (fun (y_pos, max_child_w, acc_h) el ->
            ...
            let w, h = draw_control parent_x y_pos max_w max_h el in
      ...
      let _, w, h =
        List.fold_left
          (fun (x_pos, acc_w, max_child_h) el ->
            ...
            let w, h = draw_control x_pos parent_y max_w max_h el in
  ...
  | Padding (l, t, r, b, d) ->
      ...
      let c_w, c_h = draw_control x_start y_start max_width max_height d in
      (c_w + l + r, c_h + t + b)
  | Other (f, d) ->
...
let draw view _ =
  ...
  let _ = draw_control 0 0 width height view in
  (* !state *)
  ()
