...
let rec draw_control parent_x parent_y parent_w parent_h state_tree = function
  | Empty -> (0, 0, state_tree)
  | Border (r, c, t, d) ->
      let w, h, state_tree =
        draw_control parent_x parent_y parent_w parent_h state_tree d
      in
      let rect =
  ...
  | Rect (w, h, r, c, d) ->
      ...
      let _ = draw_control parent_x parent_y w h state_tree d in
      (w, h, state_tree)
  | Column lst ->
      let _, w, h, state_tree =
        List.fold_left
          (fun (y_pos, max_child_w, acc_h, state_tree) el ->
            ...
            let w, h, state_tree =
              draw_control parent_x y_pos max_w max_h state_tree el
            in
      ...
      let _, w, h, state_tree =
        List.fold_left
          (fun (x_pos, acc_w, max_child_h, state_tree) el ->
            ...
            let w, h, state_tree =
              draw_control x_pos parent_y max_w max_h state_tree el
            in
  ...
  | Padding (l, t, r, b, d) ->
      ...
      let c_w, c_h, state_tree =
        draw_control x_start y_start max_width max_height state_tree d
      in
      (c_w + l + r, c_h + t + b, state_tree)
  | Other (f, d) ->
      let w, h = f parent_x parent_y parent_w parent_h in
      let _, _, state_tree = draw_control parent_x parent_y w h state_tree d in
      (w, h, state_tree)
...
let draw view state_tree =
  ...
  let _, _, state_tree = draw_control 0 0 width height state_tree view in
  state_tree
