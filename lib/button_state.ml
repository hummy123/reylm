...
let reduce state = function
  | Hover ->
      let easing = if state.easing <= 0.9 then 0.9 else state.easing -. 0.05 in
      { action = Hover; easing }
  | ClickHeld ->
      let easing = if state.easing <= 0.8 then 0.8 else state.easing -. 0.05 in
      { action = ClickHeld; easing }
  | Inactive ->
      if state.easing >= 1.0 then initial
      else { state with easing = state.easing +. 0.05 }
