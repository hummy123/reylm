...
let get_direction_size caller (constraints : input_constraints) =
  match caller with
  | Row -> constraints.max_width
  | Column -> constraints.max_height

let update_get_direction_size caller constraints =
  match caller with
  | Row -> constraints.max_width
  | Column -> constraints.max_height
...
let split_children caller wrap_type line_padding children
...
let update_split_children caller wrap_type line_padding children
    (constraints : 'a state_constraints) =
  let direction_size = update_get_direction_size caller constraints in
  let rec get_line pos acc_els acc_size =
    if pos = Array.length children then
      let line =
        wrap_inner caller wrap_type (List.rev acc_els |> Array.of_list)
      ...
      let child = Array.unsafe_get children pos in
      let child_size =
        size (drop_model constraints) child |> get_child_size caller
      in
      if child_size + acc_size <= direction_size then
        get_line (pos + 1) (child :: acc_els) (acc_size + child_size)
      else
...
let update_row wrap_type row_padding children constraints =
  let children =
    update_split_children Row wrap_type row_padding children constraints
  in
  Drawable.update_model constraints children
...
let update_column wrap_type col_padding children constraints =
  let children =
    update_split_children Column wrap_type col_padding children constraints
  in
  update_model constraints children
