...
let update_set_start_pos (constraints : 'a state_constraints) start_pos model =
  function
  | Column -> { constraints with start_y = start_pos; model }
  | Row -> { constraints with start_x = start_pos; model }
        ...
        let constraints =
          ...
          | Flex (flex_value, (Expand | FillHeight | FillWidth), _) ->
              ...
              let el_size =
                remaining_space *. flex_percent |> Float.round |> int_of_float
              in
              update_set_flex_child_constraints model constraints el_size
                start_pos caller
          | _ -> update_set_start_pos constraints start_pos caller model
        in
        let output = Drawable.update_model { constraints with model } el in
        let start_pos = update_increment_start_pos start_pos output caller in
        (start_pos, output.model))
...
*)
let collapse_constraints caller should_collapse flex_data
...
let update_collapse_constraints caller should_collapse flex_data constraints =
  if should_collapse then
    match caller with
    | Column -> { constraints with max_width = flex_data.max_child_width }
    | Row -> { constraints with max_height = flex_data.max_child_height }
...
let calc_min_size caller flex_data (constraints : input_constraints) =
  match caller with
  | Column ->
      let width =
...
let min_update children caller constraints =
  let flex_data = Flex.calc_flex_data children (drop_model constraints) in
  if is_in_flex_direction flex_data caller then
    ...
    let _, model =
      Array.fold_left
        (fun (start_pos, model) el ->
          let constraints =
            update_set_start_pos constraints start_pos caller model
          in
          let output = Drawable.update_model constraints el in
          (update_increment_start_pos start_pos output caller, output.model))
        (update_get_start_pos constraints caller, constraints.model)
        children
    in
    let ({ width; height } : drawable_size) =
      calc_min_size caller flex_data (drop_model constraints)
    in
    { width; height; model }
...
let flex_update_if_flex_children should_collapse children caller constraints
  ...
  let flex_data = Flex.calc_flex_data children (drop_model constraints) in
  let constraints =
    update_collapse_constraints caller should_collapse flex_data constraints
  in
  match caller with
  | Column ->
      if flex_data.num_flex_height_children > 0 then
...
let directional_update calc_start should_collapse children caller constraints =
  let if_not_flex flex_data constraints =
    let start_pos =
      update_get_start_pos constraints caller + calc_start constraints flex_data
    in
    let _, model =
      Array.fold_left
        (fun (start_pos, model) el ->
  ...
  in
  flex_update_if_flex_children should_collapse children caller constraints
    if_not_flex

let calc_when_start _ _ = 0

let calc_when_center caller (constraints : input_constraints) flex_data =
  match caller with
  | Column ->
...
let update_calc_when_center caller constraints flex_data =
  match caller with
  | Column ->
      (constraints.max_height / 2) - (flex_data.occupied_non_flex_height / 2)
  | Row -> (constraints.max_width / 2) - (flex_data.occupied_non_flex_width / 2)

let calc_when_end caller (constraints : input_constraints) flex_data =
  match caller with
...
let update_calc_when_end caller constraints flex_data =
  match caller with
  | Column -> constraints.max_height - flex_data.occupied_non_flex_height
  | Row -> constraints.max_width - flex_data.occupied_non_flex_width
...
let update_space_between should_collapse children caller constraints =
  let if_not_flex _ constraints =
    let spacer = get_spacer caller in
    (* We will insert a spacer in between each child. *)
    let children =
...
let update_space_around should_collapse children caller constraints =
  (* if_not_flex function has to add spacing by itself,
     because the same spacer trick used in space_between doesn't work for space_around. *)
  let if_not_flex flex_data constraints =
    let remaining_space =
    ...
    let _, model =
      Array.fold_left
        (fun (start_pos, model) el ->
          ...
          in
          let output = Drawable.update_model constraints el in
          ( update_increment_start_pos (start_pos + space) output caller,
            output.model ))
        (start_pos + (space / 2), constraints.model)
        children
    in
    { width = constraints.max_width; height = constraints.max_height; model }
  in
