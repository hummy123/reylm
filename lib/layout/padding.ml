...
let get_child_constraints l t r b (constraints : input_constraints) =
...
let update_get_child_constraints l t r b constraints =
  let max_width = constraints.max_width - l - r in
  let min_width = constraints.min_width - l - r in
  let max_height = constraints.max_height - t - b in
  let min_height = constraints.min_height - t - r in
...
let update l t r b child constraints =
  let child_constraints = update_get_child_constraints l t r b constraints in
  let { width; height; model } =
    Drawable.update_model child_constraints child
  in
