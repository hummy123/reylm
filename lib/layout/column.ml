...
let calc_start_y_center (constraints : input_constraints) flex_data =
  (constraints.max_height / 2) - (flex_data.occupied_non_flex_height / 2)

let update_calc_start_y_center constraints flex_data =
  (constraints.max_height / 2) - (flex_data.occupied_non_flex_height / 2)
  ...
  Widget
    ( Column_row.directional_draw
      ...
      Column_row.directional_update
        (Column_row.update_calc_when_center Column)
        collapse_width children Column )
...
let bottom ?(collapse_width = true) children =
  Widget
    ( Column_row.directional_draw
      ...
      Column_row.max_size collapse_width children Column,
      Column_row.directional_update
        (Column_row.update_calc_when_end Column)
        collapse_width children Column )
